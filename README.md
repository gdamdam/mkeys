...
- **Sustain vs. latch** — these are separate. **Latch** (Play → Chord section) is a performance toggle that keeps played notes sounding after release. **Sustain** is the MIDI pedal (CC64): while it's down, note-offs are deferred and released when the pedal lifts — for keys no longer held; keys still down keep sounding. Re-striking a sustained note retriggers it. The pedal never flips the on-screen Latch toggle.
...
- **MIDI routing** — the transport **Routing** section selects a specific **input** device (or *All inputs*) with an **input-channel filter** (**Omni** or 1–16; Omni is required for MPE-in), and a single **output** device (*Auto* = first available; there is deliberately no "all outputs" broadcast). Selections persist by stable device id and survive reconnects; a disconnected saved device is shown as such and routed **nowhere** rather than silently elsewhere. Hotplug is tracked via `onstatechange`, changing the output device/channel/MPE/enablement sends Panic to the affected destination first, and a likely feedback-loop (same in+out device) is flagged.
- **Play quantize (timing)** — a Perform-panel section, **distinct from Glide quantize**: it controls *when* notes land on the musical grid, not pitch. **Timing mode**: *Immediate* (notes sound instantly), *Quantized rec* (monitoring stays live; captured phrase notes snap to the grid — best for tight loops), or *Quantized live* (a press waits for the next grid boundary before sounding, with a pending highlight on the surface). **Grid**: Off · 1/16 · 1/8 · 1/4 · Beat · Bar. Uses the audio/scheduler clock and the shared Link beat/phase when Link is enabled; changing BPM preserves musical phase. Old sessions/links default to Immediate / Off. The arpeggiator is treated as already tempo-quantized (no second layer).
- **Glide quantize (pitch)** — a **Glide quantize** slider in the Perform panel (Playing surface) sets how a slide moves between degrees: **0% = continuous** portamento, **100% = stepped/snap**. Applied live. The same panel's *Advanced layout* exposes rows, columns and the isomorphic row offset.
